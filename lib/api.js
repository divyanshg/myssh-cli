...
export function authHeaders() {
  // Check for service token first (CI/CD mode)
  const serviceToken = process.env.MYSSH_SERVICE_TOKEN;
  if (serviceToken) {
    return { Authorization: `Bearer ${serviceToken}` };
  }

  // Fall back to user token from config
...
// ─── Vault Personal Secrets ───

export async function listPersonalSecrets(orgId, projectSlug, envSlug) {
  orgId = orgId || requireOrgId();
  const { data } = await api.get(`/api/orgs/${orgId}/vault/projects/${projectSlug}/environments/${envSlug}/personal-secrets`, { headers: authHeaders() });
  return data;
}

export async function getPersonalSecret(orgId, projectSlug, envSlug, key) {
  orgId = orgId || requireOrgId();
  const { data } = await api.get(`/api/orgs/${orgId}/vault/projects/${projectSlug}/environments/${envSlug}/personal-secrets/${encodeURIComponent(key)}`, { headers: authHeaders() });
  return data;
}

export async function setPersonalSecret(orgId, projectSlug, envSlug, key, value) {
  orgId = orgId || requireOrgId();
  const { data } = await api.put(`/api/orgs/${orgId}/vault/projects/${projectSlug}/environments/${envSlug}/personal-secrets/${encodeURIComponent(key)}`, { value }, { headers: authHeaders() });
  return data;
}

export async function deletePersonalSecret(orgId, projectSlug, envSlug, key) {
  orgId = orgId || requireOrgId();
  const { data } = await api.delete(`/api/orgs/${orgId}/vault/projects/${projectSlug}/environments/${envSlug}/personal-secrets/${encodeURIComponent(key)}`, { headers: authHeaders() });
  return data;
}
...
export async function createSSHToken(orgId, { name, allowedNodeIds, expiresAt }) {
  orgId = orgId || requireOrgId();
  const { data } = await api.post(`/api/orgs/${orgId}/ssh-tokens`, {
    name,
    allowedNodeIds,
    expiresAt,
  }, { headers: authHeaders() });
...
export async function listSSHTokens(orgId) {
  orgId = orgId || requireOrgId();
  const { data } = await api.get(`/api/orgs/${orgId}/ssh-tokens`, { headers: authHeaders() });
  return data;
}

export async function revokeSSHToken(orgId, tokenId) {
  orgId = orgId || requireOrgId();
  const { data } = await api.delete(`/api/orgs/${orgId}/ssh-tokens/${tokenId}`, { headers: authHeaders() });
  return data;
}
