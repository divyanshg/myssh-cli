...
export async function listVaultSecrets(orgId, projectSlug, envSlug) {
  orgId = orgId || requireOrgId();
  const { data } = await api.get(`/api/orgs/${orgId}/vault/projects/${projectSlug}/environments/${envSlug}/secrets`, { headers: authHeaders() });
  return data;
}
...
export async function setVaultSecret(orgId, projectSlug, envSlug, key, value) {
  orgId = orgId || requireOrgId();
  const { data } = await api.put(`/api/orgs/${orgId}/vault/projects/${projectSlug}/environments/${envSlug}/secrets/${encodeURIComponent(key)}`, { value }, { headers: authHeaders() });
  return data;
}
