...
export function requireOrgId() {
  // Check for environment variable first (CI/CD mode)
  const envOrgId = process.env.MYSSH_ORG_ID;
  if (envOrgId) {
    return envOrgId;
  }
  ...
  if (!orgId) {
    console.error('No active organization. Run: myssh org switch <orgId|slug> or set MYSSH_ORG_ID');
    process.exit(1);
  }
  return orgId;
}
