...
import {
  nodeRequireTotpCommand,
  nodeUnrequireTotpCommand,
} from "../commands/node-totp.js";
...
import {
  vaultSealStatusCommand,
  vaultInitCommand,
  vaultUnsealCommand,
  vaultSealCommand,
  vaultUnwrapCommand,
} from "../commands/vault-seal.js";
import {
  vaultKeyRotateCommand,
...
org.command("ls").description("List your organizations").action(orgListCommand);

org
  .command("switch <orgIdOrSlug>")
...
// ── Node commands ──────────────────────────────────────
const node = program.command("node").description("Manage registered nodes");
...
node
  .command("transfer <nodeId> <targetOrgId>")
  .description("Transfer a node to another organization")
  .action(nodeTransferCommand);

node
  .command("require-totp <nodeId>")
  .description("Require TOTP authentication for connecting to a node")
  .action(nodeRequireTotpCommand);

node
  .command("unrequire-totp <nodeId>")
  .description("Remove TOTP requirement from a node")
  .action(nodeUnrequireTotpCommand);
...
access
  .command("grant <nodeId> <email>")
  .description("Grant a user access to a node")
  .option(
    "-p, --permission <permission>",
    "Permission level (CONNECT)",
    "CONNECT",
  )
  .option(
    "-d, --duration <minutes>",
    "Access duration in minutes (e.g. 60, 1440 for 1 day)",
  )
  .action(nodeAccessGrantCommand);

access
  .command("revoke <nodeId> <email>")
...
program
  ...
  .description(
    "Get a certificate and SSH into a node (accepts node ID or hostname)",
  )
  .option("-d, --direct", "Connect directly to the node (skip proxy)")
  .option(
    "-t, --ttl <minutes>",
    "Session TTL in minutes (1-60, default: 30)",
    "30",
  )
...
forward
  .command("start <nodeIdOrHostname> <remotePort> [localPort]")
  .description(
    "Forward a remote port to localhost (e.g. forward start myserver 5432 3000)",
  )
  .option(
    "-H, --remote-host <host>",
    "Remote bind address on the node",
    "127.0.0.1",
  )
...
vault
  .command("run")
  .description(
    "Inject secrets and run a command (e.g. myssh vault run -- npm start)",
  )
...
const vaultEnv = vault.command("env").description("Manage vault environments");
...
vault
  ...
  .option("--ttl <seconds>", "Secret TTL in seconds (min 60)")
  .option("--expires-at <datetime>", "Absolute expiration (ISO 8601)")
...
vault
  ...
  .option("-p, --project <slug>", "Project slug")
  .option("-e, --env <slug>", "Environment slug")
  .option("--include-deleted", "Include soft-deleted secrets")
...
// ── Vault restore / purge / rollback ───────────────────
vault
  .command("restore <key>")
  .description("Restore a soft-deleted secret")
  .option("--org <orgId>", "Organization ID")
  .option("-p, --project <slug>", "Project slug")
  .option("-e, --env <slug>", "Environment slug")
  .action(vaultRestoreCommand);

vault
  .command("purge <key>")
  .description("Permanently delete a secret (admin only)")
  .option("--org <orgId>", "Organization ID")
  .option("-p, --project <slug>", "Project slug")
  .option("-e, --env <slug>", "Environment slug")
  .action(vaultPurgeCommand);
...
vault
  ...
  .option("-p, --project <slug>", "Project slug")
  .option("-e, --env <slug>", "Environment slug")
  .action(vaultRollbackCommand);

// ── Vault seal / unseal sub-commands ───────────────────
const seal = vault.command("seal").description("Manage vault seal state");

seal
  .command("status")
  .description("Show vault seal status")
  .action(vaultSealStatusCommand);

seal
  .command("init")
  .description("Initialize vault with Shamir key shares")
  .option("-t, --threshold <n>", "Minimum shares to unseal", "3")
  .option("-s, --shares <n>", "Total number of shares", "5")
  .action(vaultInitCommand);

seal
  .command("unseal <share>")
  .description("Submit an unseal share")
  .action(vaultUnsealCommand);

seal
  .command("lock")
  .description("Seal the vault (clears master key from memory)")
  .action(vaultSealCommand);

vault
  .command("unwrap <token>")
  .description("Unwrap a single-use wrapped response token")
  .action(vaultUnwrapCommand);

// ── Vault key sub-commands ─────────────────────────────
const key = vault.command("key").description("Manage encryption keys");

key
  .command("rotate")
  .description("Rotate the organization encryption key (owner only)")
  .action(vaultKeyRotateCommand);

key
  .command("re-encrypt")
  .description("Re-encrypt all secrets with the latest key")
  .option("--batch-size <n>", "Number of secrets per batch")
  .action(vaultKeyReEncryptCommand);
